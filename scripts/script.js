...
document.addEventListener("DOMContentLoaded", function () {
  const formConsaltDesk = document.getElementById("consalt-form-desktop");

  formConsaltDesk.addEventListener("submit", formSend);

  async function formSend(e) {
    e.preventDefault();

    let error = formValidate(formConsaltDesk);

    let formData = new FormData(formConsaltDesk);
  ...
  function formValidate(formConsaltDesk) {
    let error = 0;
